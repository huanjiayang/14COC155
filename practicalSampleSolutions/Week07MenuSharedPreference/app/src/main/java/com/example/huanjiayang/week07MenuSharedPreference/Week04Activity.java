package com.example.huanjiayang.week07MenuSharedPreference;
...
public class Week04Activity extends ActionBarActivity {
    ...
    protected void onStart() {
        ...
        // We also refresh the weather when app starts
        // because we know the city preference in the settings,
        // we don't have to wait for user to enter one.
        // so the user will no longer see dummy data on start
        refreshWeather();
    }
    ...
    // The same function as before, just not taking any parameter
    public void seeOnMap(){
        ...
        // Instead of looking at the EditText, we get the city name value from the default SharedPreferences
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_settings) {
            // Use explicit Intent to move to the settings Activity
            Intent settingIntent = new Intent(getApplicationContext(),Week07SettingsActivity.class);
            startActivity(settingIntent);
            return true;
        }
        else if (id == R.id.refresh_button){
            // Refresh weather when user clicks refresh item
            refreshWeather();
        }
        else if (id == R.id.view_on_map){
            // open map to view location
            seeOnMap();
        }
