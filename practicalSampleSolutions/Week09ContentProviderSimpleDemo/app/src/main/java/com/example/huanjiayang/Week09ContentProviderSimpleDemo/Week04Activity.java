...
import com.example.huanjiayang.Week09ContentProviderSimpleDemo.data.WeatherDataModel;
...
*/
        String[] columns = {
                ...
                WeatherDataModel.weatherEntry.COLUMN_DAY_NO,
                WeatherDataModel.weatherEntry.COLUMN_DAY_WEATHER
        };
// A cursor is your primary interface to the query results.
        Cursor cursor = getApplicationContext().getContentResolver().query(
                WeatherDataModel.weatherEntry.CONTENT_URI, // Uri to Query
                columns,
...
// Get the value in each column by finding the appropriate column index.
            do {
                ...
                int id_index = cursor.getColumnIndex(WeatherDataModel.weatherEntry._ID);
                Log.i("Week09WeatherProvider","reading row with _ID: "+cursor.getInt(id_index));
                int day_no_index = cursor.getColumnIndex(WeatherDataModel.weatherEntry.COLUMN_DAY_NO);
                Log.i("Week09WeatherProvider","reading row with DAY_NO: "+cursor.getInt(day_no_index));
...
// Create a new map of values, where column names are the keys
            ...
            for(int i=0;i<myArrayList.size();i++) {
                values.put(WeatherDataModel.weatherEntry.COLUMN_DAY_WEATHER, myArrayList.get(i).toString());
                values.put(WeatherDataModel.weatherEntry.COLUMN_DAY_NO, i);
